...
def readTrainingData(dir):
    data = np.array([json.load(open("c:/users/ben/desktop/file.json"))])
    labels = np.array([1 for d in data])
    return data, labels
...
# max pooling.
def max_pool_2x2(x):
    return tf.nn.max_pool(x, ksize=[2, 4, 4, 2], strides=[2, 4, 4, 2], padding="SAME")
...
W_conv1 = createWeight([25, 25, 1, 64])
